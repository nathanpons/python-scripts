...
class HoldKeyUI:
    ...
    def setup_ui(self):
      ...
      # Title
      ttk.Label(self.parent_frame, text="Hold Key Script").pack(pady=10)
      ...
      self.hold_key_combobox = ttk.Combobox(config_frame, textvariable=self.hold_key_var, values=self.hold_keys, state="readonly")
      ...
      self.toggle_key_combobox = ttk.Combobox(config_frame, textvariable=self.toggle_key_var, values=self.toggle_keys, state="readonly")
      ...
      self.toggle_script_button = ttk.Button(button_frame, textvariable=self.toggle_script_button_text, command=self.toggle_script)
    ...
    def toggle_script(self):
      ...
      if self.script is None or not self.script.running:
        ...
        self.script = HoldKeyScript(hold_key=hold_key, toggle_key=toggle_key)
        self.script.start()
